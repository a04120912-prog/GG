...
/* =====================================================
   챔피언 분석 탭
   ===================================================== */
function ChampionAnalysis({ allStats, matches }) {
  ...
  const champStats = (() => {
    ...
    const laneGameCount = laneFilter === 'ALL'
      ? totalGames
      : new Set(
          allStats
            .filter(s => String(s.lane || '').toUpperCase().trim() === laneFilter)
            .map(s => s.match_id)
        ).size;
    ...
    allStats.forEach(s => {
      ...
      if (!map[name]) {
        map[name] = { name, pickCount: 0, wins: 0, totalDmg: 0, totalDmgTaken: 0, totalGold: 0, totalCs: 0, totalVs: 0, totalControlWards: 0, totalK: 0, totalD: 0, totalA: 0, totalMin: 0, lanes: {} };
      }
    ...
    return Object.values(map).map(c => {
      ...
      const banRate = (() => {
        ...
        matches.forEach(m => {
          ...
          if (allBans.some(b => {
            if (b.champ !== c.name) return false;
            if (laneFilter === 'ALL') return true;
            return String(b.target || '').toUpperCase().trim() === laneFilter;
          })) banCount++;
        });
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '10px', marginBottom: '24px' }}>
        {lanes.map(lane => (
          <button key={lane} onClick={() => setLaneFilter(lane)} style={{ padding: '8px 18px', borderRadius: '10px', border: laneFilter === lane ? '1px solid #60a5fa' : '1px solid #374151', cursor: 'pointer', fontSize: '13px', fontWeight: 'bold', transition: '0.2s', backgroundColor: laneFilter === lane ? '#3b82f6' : '#111827', color: laneFilter === lane ? '#fff' : '#9ca3af' }}>{lane}</button>
        ))}
...
/* =====================================================
   개인 지표 탭
   ===================================================== */
function PlayerReport({ selectedPlayer, setSelectedPlayer, allStats, matches, currentData, radarData, reportType, setReportType, selectedLine, setSelectedLine, dataScope, setDataScope, selectedChampion, setSelectedChampion, getRankingsByLine }) {
  ...
  if (!selectedPlayer || !currentData) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '400px', gap: '16px', color: '#4b5563' }}>
        ...
        <p style={{ fontSize: '14px', color: '#4b5563' }}>전적 검색 탭의 경기 기록에서 플레이어를 클릭하거나,<br/>아래 검색창에서 직접 찾아보세요.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '25px' }}>
        <LineTab label="전체" active={selectedLine === 'ALL'} count={selectedPlayer.fullHistory.length} winRate={Math.round((selectedPlayer.fullHistory.filter(h => h.isWin).length / selectedPlayer.fullHistory.length) * 100)} onClick={() => { setSelectedLine('ALL'); setSelectedChampion(null); }} />
        {['TOP', 'JNG', 'MID', 'ADC', 'SUP'].map(lane => {
          const summary = selectedPlayer.lineSummary[lane];
          if (!summary) return null;
          return <LineTab key={lane} label={lane} active={selectedLine === lane} count={summary.count} winRate={Math.round((summary.wins / summary.count) * 100)} onClick={() => { setSelectedLine(lane); setSelectedChampion(null); }} />;
        })}
      </div>
        ...
        return sortedBans.length > 0 ? sortedBans.map(([name, count]) => {
          ...
          return (
            <div key={name} style={{ textAlign: 'center' }}>
              <div style={{ position: 'relative', display: 'inline-block' }}>
                <img src={getChampImgUrl(name)} style={{ width: '45px', height: '45px', borderRadius: '10px', border: 'none', filter: 'grayscale(0.8)', boxShadow: isSignature ? '0 0 0 2px #f97316, 0 0 10px rgba(249,115,22,0.5)' : '0 0 0 1px #ef4444' }} alt={name} />
...
/* =====================================================
   상대 전적 탭
   ===================================================== */
function HeadToHead({ allStats, matches, onNavigateToPlayer }) {
  ...
  const rivalData = (() => {
    ...
    myStats.forEach(s => {
      ...
      const opponents = allStats.filter(op =>
        op.match_id === matchId &&
        op.nickname !== playerA &&
        String(op.side || '').trim() !== mySide &&
        String(op.lane || '').toUpperCase().trim() === myLane
      );
      ...
      opponents.forEach(op => {
        ...
        rivalMap[opName].byLane[myLane].push({
          ...
          my: {
            ...
            damage: Number(s.damage || 0), gold: Number(s.gold || 0), cs: Number(s.cs || 0),
            dpm: Math.round(Number(s.damage || 0) / mTotal), gpm: Math.round(Number(s.gold || 0) / mTotal),
          },
          op: {
            champion: op.champion,
            kills: Number(op.kills || 0), deaths: Number(op.deaths || 0), assists: Number(op.assists || 0),
            damage: Number(op.damage || 0), gold: Number(op.gold || 0), cs: Number(op.cs || 0),
            dpm: Math.round(Number(op.damage || 0) / mTotal), gpm: Math.round(Number(op.gold || 0) / mTotal),
          },
        });
      });
    });
    ...
    const calcLaneStats = (games) => {
      ...
      const myTotK = games.reduce((s, g) => s + g.my.kills, 0);
      const myTotD = games.reduce((s, g) => s + g.my.deaths, 0);
      const myTotA = games.reduce((s, g) => s + g.my.assists, 0);
      const opTotK = games.reduce((s, g) => s + g.op.kills, 0);
      const opTotD = games.reduce((s, g) => s + g.op.deaths, 0);
      const opTotA = games.reduce((s, g) => s + g.op.assists, 0);
      const myKdaNum = myTotD === 0 ? 9999 : (myTotK + myTotA) / myTotD;
      const opKdaNum = opTotD === 0 ? 9999 : (opTotK + opTotA) / opTotD;
      return {
        games,
        total: games.length, wins, losses: games.length - wins,
        winRate: Math.round((wins / games.length) * 100),
        myAvgDpm: Math.round(games.reduce((s, g) => s + g.my.dpm, 0) / n),
        opAvgDpm: Math.round(games.reduce((s, g) => s + g.op.dpm, 0) / n),
        myAvgGpm: Math.round(games.reduce((s, g) => s + g.my.gpm, 0) / n),
        opAvgGpm: Math.round(games.reduce((s, g) => s + g.op.gpm, 0) / n),
        myAvgCs: (games.reduce((s, g) => s + g.my.cs, 0) / n).toFixed(1),
        opAvgCs: (games.reduce((s, g) => s + g.op.cs, 0) / n).toFixed(1),
        myKda: myTotD === 0 ? 'Perfect' : ((myTotK + myTotA) / myTotD).toFixed(2),
        opKda: opTotD === 0 ? 'Perfect' : ((opTotK + opTotA) / opTotD).toFixed(2),
        myKdaNum, opKdaNum,
      };
    };
    ...
    return Object.values(rivalMap).map(r => {
      ...
      return {
        nickname: r.nickname,
        lanes, laneStats, overall, mostLane,
        total: overall.total, wins: overall.wins, losses: overall.losses, winRate: overall.winRate,
        myAvgDpm: overall.myAvgDpm, opAvgDpm: overall.opAvgDpm,
        myAvgGpm: overall.myAvgGpm, opAvgGpm: overall.opAvgGpm,
        myAvgCs: overall.myAvgCs, opAvgCs: overall.opAvgCs,
        latestOpChamp: [...allGames].sort((a, b) => new Date(b.date) - new Date(a.date))[0]?.op.champion,
      };
    });
  })();

  const sorted = rivalData
    ? [...rivalData].sort((a, b) => {
  ...
  const MiniBar = ({ myVal, opVal }) => {
    ...
    const b = parseFloat(opVal) || 0;
    const total = a + b || 1;
    const pct = (a / total) * 100;
    const myBetter = a >= b;
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '12px' }}>
        <span style={{ color: myBetter ? '#60a5fa' : '#9ca3af', fontWeight: myBetter ? 'bold' : 'normal', width: '50px', textAlign: 'right' }}>
          {typeof myVal === 'number' ? myVal.toLocaleString() : myVal}
        </span>
        ...
        <span style={{ color: !myBetter ? '#f87171' : '#9ca3af', fontWeight: !myBetter ? 'bold' : 'normal', width: '50px' }}>
          {typeof opVal === 'number' ? opVal.toLocaleString() : opVal}
        </span>
      </div>
    );
  };
  ...
  const StatBar = ({ label, myVal, opVal, myNum, opNum }) => {
    const total = myNum + opNum || 1;
    const pctA = (myNum / total) * 100;
    const aIsBetter = myNum >= opNum;
    return (
      <div style={{ marginBottom: '16px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '13px' }}>
          <span style={{ fontWeight: 'bold', color: aIsBetter ? '#60a5fa' : '#9ca3af' }}>
            {typeof myVal === 'number' ? myVal.toLocaleString() : myVal}
          </span>
          <span style={{ color: '#6b7280', fontSize: '11px' }}>{label}</span>
          <span style={{ fontWeight: 'bold', color: !aIsBetter ? '#f87171' : '#9ca3af' }}>
            {typeof opVal === 'number' ? opVal.toLocaleString() : opVal}
          </span>
        </div>
  ...
  const DetailPanel = ({ r }) => {
    ...
    return (
      <div style={{ backgroundColor: '#111827', borderTop: '1px solid #374151', padding: '24px' }}>
        ...
        {multiLane && (
          <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
            {r.lanes.map(lane => (
              <button key={lane}
                onClick={() => setSelectedLaneTab(prev => ({ ...prev, [r.nickname]: lane }))}
                style={{
                  padding: '6px 16px', borderRadius: '8px', cursor: 'pointer',
                  fontSize: '12px', fontWeight: 'bold', transition: '0.2s',
                  backgroundColor: activeLane === lane ? '#3b82f6' : '#1f2937',
                  color: activeLane === lane ? '#fff' : '#9ca3af',
                  border: activeLane === lane ? '1px solid #60a5fa' : '1px solid #374151',
                }}
              >
            ...
            <button
              onClick={() => setSelectedLaneTab(prev => ({ ...prev, [r.nickname]: 'ALL' }))}
              style={{
                padding: '6px 16px', borderRadius: '8px', cursor: 'pointer',
                fontSize: '12px', fontWeight: 'bold', transition: '0.2s',
                backgroundColor: activeLane === 'ALL' ? '#6b21a8' : '#1f2937',
                color: activeLane === 'ALL' ? '#fff' : '#9ca3af',
                border: activeLane === 'ALL' ? '1px solid #a855f7' : '1px solid #374151',
              }}
            >
              전체 ({r.overall.total})
            </button>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', backgroundColor: '#1f2937', borderRadius: '16px', overflow: 'hidden', border: '1px solid #374151', marginBottom: '20px' }}>
          <div style={{ padding: '24px', textAlign: 'center', background: 'linear-gradient(135deg, rgba(59,130,246,0.15) 0%, transparent 100%)' }}>
            <div style={{ fontSize: '18px', fontWeight: '900', color: '#60a5fa', marginBottom: '16px' }}>{playerA}</div>
            <div style={{ fontSize: '48px', fontWeight: '900', color: st.wins >= st.losses ? '#60a5fa' : '#4b5563', lineHeight: 1 }}>{st.wins}</div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px 28px', borderLeft: '1px solid #374151', borderRight: '1px solid #374151', minWidth: '110px' }}>
            <div style={{ fontSize: '13px', color: '#6b7280', marginBottom: '6px' }}>대면 경기</div>
            <div style={{ fontSize: '32px', fontWeight: '900', color: '#fff' }}>{st.total}</div>
            {multiLane && activeLane !== 'ALL' && (
              <div style={{ fontSize: '11px', color: '#a855f7', marginTop: '4px' }}>{activeLane} 라인</div>
            )}
            <div style={{ marginTop: '14px', fontSize: '13px', fontWeight: 'bold', color: st.wins > st.losses ? '#60a5fa' : st.wins < st.losses ? '#f87171' : '#9ca3af' }}>
              {st.wins > st.losses ? `${playerA} 우세` : st.wins < st.losses ? `${r.nickname} 우세` : '동률'}
            </div>
          </div>
          <div style={{ padding: '24px', textAlign: 'center', background: 'linear-gradient(225deg, rgba(239,68,68,0.15) 0%, transparent 100%)' }}>
            <div style={{ fontSize: '18px', fontWeight: '900', color: '#f87171', marginBottom: '16px' }}>{r.nickname}</div>
            <div style={{ fontSize: '48px', fontWeight: '900', color: st.losses >= st.wins ? '#f87171' : '#4b5563', lineHeight: 1 }}>{st.losses}</div>
            <div style={{ fontSize: '13px', color: '#6b7280', marginTop: '4px' }}>승리</div>
            <div style={{ marginTop: '14px', display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '13px' }}>
        ...
        <div style={{ backgroundColor: '#1f2937', borderRadius: '14px', padding: '20px 24px', border: '1px solid #374151', marginBottom: '20px' }}>
          <h4 style={{ fontSize: '14px', fontWeight: 'bold', color: '#fff', marginBottom: '18px' }}>
            📊 지표 비교
            <span style={{ fontSize: '11px', color: '#4b5563', fontWeight: 'normal', marginLeft: '8px' }}>
              ({st.total}경기 평균{multiLane && activeLane !== 'ALL' ? ` · ${activeLane}` : ''})
            </span>
          </h4>
          ...
          <div style={{ display: 'flex', gap: '20px', marginTop: '14px', paddingTop: '14px', borderTop: '1px solid #1f2937' }}>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '28px' }}>
        <div style={{ fontSize: '13px', color: '#9ca3af', marginBottom: '10px', fontWeight: 'bold' }}>
          📌 플레이어 선택 — 상대 팀 같은 라인 전적만 집계됩니다
        </div>
        {playerA ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', backgroundColor: '#111827', padding: '14px 20px', borderRadius: '12px', border: '2px solid #3b82f6', maxWidth: '400px' }}>
            ...
            <button onClick={() => { setPlayerA(''); setSearchA(''); setExpandedRival(null); setSelectedLaneTab({}); }}
              style={{ background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', fontSize: '18px' }}>✕</button>
          </div>
        ) : (
          <div style={{ position: 'relative', maxWidth: '400px' }}>
            <input
              type="text" placeholder="플레이어 닉네임 검색" value={searchA}
              onChange={e => setSearchA(e.target.value)} autoFocus
              style={{ width: '100%', backgroundColor: '#111827', border: '2px solid #3b82f6', borderRadius: '12px', padding: '13px 18px', color: '#fff', fontSize: '14px', outline: 'none', boxSizing: 'border-box' }}
            />
            {searchA && (
              <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, backgroundColor: '#1f2937', border: '1px solid #3b82f644', borderRadius: '10px', marginTop: '4px', zIndex: 10, maxHeight: '220px', overflowY: 'auto' }}>
                {allNicknames.filter(n => n.toLowerCase().includes(searchA.toLowerCase())).map(n => (
                  <div key={n} onClick={() => { setPlayerA(n); setSearchA(''); }}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ display: 'flex', gap: '20px', marginBottom: '4px', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div style={{ width: '12px', height: '5px', borderRadius: '3px', backgroundColor: '#3b82f6' }} />
              <span style={{ fontSize: '12px', color: '#6b7280' }}>{playerA}</span>
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
              <div style={{ width: '12px', height: '5px', borderRadius: '3px', backgroundColor: '#ef4444' }} />
              <span style={{ fontSize: '12px', color: '#6b7280' }}>상대</span>
            </div>
          ...
          {sorted.map((r, i) => {
            ...
            return (
              <div key={r.nickname} style={{ borderRadius: '16px', border: isExpanded ? '2px solid #3b82f6' : '1px solid #374151', overflow: 'hidden', transition: '0.2s' }}>
                ...
                {/* 요약 행 */}
                <div
                  onClick={() => setExpandedRival(isExpanded ? null : r.nickname)}
                  style={{ display: 'grid', gridTemplateColumns: '200px 1fr', alignItems: 'center', backgroundColor: isExpanded ? 'rgba(59,130,246,0.1)' : i % 2 === 0 ? '#1a2030' : '#1f2937', cursor: 'pointer', padding: '14px 20px', transition: '0.15s' }}
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '24px', justifyContent: 'flex-end' }}>
                    <div style={{ textAlign: 'center' }}>
                      ...
                      <div style={{ fontSize: '15px', fontWeight: 'bold' }}>
                        <span style={{ color: '#60a5fa' }}>{r.wins}</span>
                    ...
                    <div style={{ textAlign: 'center', minWidth: '110px' }}>
                      <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '4px' }}>DPM</div>
                      <MiniBar myVal={r.myAvgDpm} opVal={r.opAvgDpm} />
                    </div>
                    <div style={{ textAlign: 'center', minWidth: '110px' }}>
                      <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '4px' }}>GPM</div>
                      <MiniBar myVal={r.myAvgGpm} opVal={r.opAvgGpm} />
                    </div>
                    <div style={{ textAlign: 'center', minWidth: '110px' }}>
                      <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '4px' }}>평균 CS</div>
                      <MiniBar myVal={r.myAvgCs} opVal={r.opAvgCs} />
                    </div>
...
function App() {
  ...
  // 메인 탭: 'search' | 'champion' | 'player' | 'h2h'
    ...
    try {
      ...
      if (sData) {
        ...
        setPlayerStats(sortedData.map(item => ({
          nickname: item.nickname || 'Unknown', damage: Number(item.damage || 0), gold: Number(item.gold || 0), 
          vision_score: Number(item.vision_score || 0), control_wards: Number(item.control_wards || 0), 
          damage_taken: Number(item.damage_taken || 0), cs: Number(item.cs || 0), kills: Number(item.kills || 0), 
          deaths: Number(item.deaths || 0), assists: Number(item.assists || 0), side: item.side, 
          firstBlood: item.first_blood, multiKill: item.multi_kill, lane: item.lane, champion: item.champion
        })));
      }
    ...
    try {
      const history = allStats.filter(s => s.nickname === nickname).map(s => {
        ...
        return { 
          ...
          champion: s.champion, match_id: s.match_id, isWin: isWinResult,
          dpm: Math.round(Number(s.damage || 0) / mTotal), dtpm: Math.round(Number(s.damage_taken || 0) / mTotal),
          dmgShare: teamTotalDmg > 0 ? Number(((Number(s.damage || 0) / teamTotalDmg) * 100).toFixed(1)) : 0,
          gpm: Math.round(Number(s.gold || 0) / mTotal), cspm: (Number(s.cs || 0) / mTotal).toFixed(1), 
          vs: Number(s.vision_score || 0), controlWards: Number(s.control_wards || 0),
          ...
          damage: Number(s.damage || 0), damage_taken: Number(s.damage_taken || 0), gold: Number(s.gold || 0), 
          cs: Number(s.cs || 0), vision_score: Number(s.vision_score || 0), matchMinutes: mTotal, 
          kills: Number(s.kills || 0), deaths: Number(s.deaths || 0), assists: Number(s.assists || 0) 
        };
      }).reverse();
  ...
  const getRadarData = (currentData, line) => {
    ...
    return keys.map(k => {
      ...
      let isPerfect = false;
      if (k.key === 'kda') {
        if (pVal === "Perfect" || String(pVal).includes("Perfect") || !isFinite(parseFloat(pVal))) isPerfect = true;
      }
  ...
  // 개인지표 탭 검색결과
  const playerSearchResults = [...new Set(allStats.map(s => s.nickname))]
    .filter(name => name.toLowerCase().includes(playerSearchTerm.toLowerCase()))
    .map(name => {
      const pHistory = allStats.filter(s => s.nickname === name);
      const wins = pHistory.filter(s => String(s.side || '').trim().toLowerCase() === String(s.matches?.win_team || '').trim().toLowerCase()).length;
      return { nickname: name, totalGames: pHistory.length, winRate: Math.round((wins / pHistory.length) * 100), mostLane: Object.entries(pHistory.reduce((acc, curr) => { const lane = String(curr.lane || 'MID').toUpperCase(); acc[lane] = (acc[lane] || 0) + 1; return acc; }, {})).sort((a, b) => b[1] - a[1])[0][0] };
  ...
  return (
    <div style={{ backgroundColor: '#0a0e17', minHeight: '100vh', width: '100%', margin: 0, padding: 0, color: '#f3f4f6', overflowX: 'hidden' }}>
      <header style={{ textAlign: 'center', padding: '80px 0 0 0', background: 'linear-gradient(to bottom, #1e293b 0%, #0a0e17 100%)', borderBottom: '1px solid #1e293b', marginBottom: '0' }}>
        <h1 style={{ fontSize: '64px', fontWeight: '900', margin: '0 0 40px 0', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '12px', lineHeight: '1.2' }}>
        ...
        {/* ★ 메인 탭 네비게이션 (4탭) */}
        <div style={{ display: 'flex', justifyContent: 'center', gap: '0', borderBottom: '2px solid #1e293b' }}>
          {[
            { id: 'search', label: '🔍 전적 검색' },
            ...
            <button
              ...
              style={{
                ...
                color: mainTab === tab.id ? '#60a5fa' : tab.id === 'player' && selectedPlayer ? '#a78bfa' : '#6b7280',
                fontSize: '15px', fontWeight: mainTab === tab.id ? '700' : '500',
                cursor: 'pointer', transition: '0.2s', marginBottom: '-2px', letterSpacing: '0.3px'
              }}
            >
              {tab.label}
            </button>
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '30px', padding: '40px 20px 100px 20px' }}>
        ...
        {mainTab === 'search' && (
          <>
              ...
              {searchTerm && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '15px', width: '100%' }}>
                  {searchResults.map(player => (
                    <div key={player.nickname} onClick={() => { handlePlayerClick(player.nickname, true); }} style={{ backgroundColor: '#111827', padding: '15px', borderRadius: '12px', border: '1px solid #374151', cursor: 'pointer' }}>
            ...
            {selectedMatchId && playerStats.length > 0 && (
              <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginTop: '20px' }}>
                {['Blue', 'Red'].map(side => {
                              ...
                              <td style={{ padding: '12px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                  ...
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', minWidth: 0, flex: 1 }}>
                                    {/* ★ 닉네임 클릭 → 개인지표 탭으로 이동 */}
                                    <span
                                      onClick={() => handlePlayerClick(p.nickname, true)}
                                      style={{ fontWeight: 'bold', color: '#fff', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '85px', cursor: 'pointer' }}
                                    >{p.nickname}</span>
                                ...
                                <div style={{ display: 'flex', gap: '6px' }}>
                                  {bans.map((ban, idx) => (
                                    <div key={idx} style={{ width: '26px', height: '26px', borderRadius: '4px', overflow: 'hidden', border: '1px solid #374151' }}>
                                      <img src={getChampImgUrl(ban.champ)} alt="ban" style={{ width: '100%', height: '100%', filter: 'grayscale(100%) opacity(0.6)' }} />
                                    </div>
                                  ))}
            ...
            <section style={{ backgroundColor: '#1f2937', padding: '35px', borderRadius: '16px' }}>
              ...
              <ResponsiveContainer height={350}>
                <BarChart data={playerStats}>
                  ...
                  <YAxis stroke="#9ca3af" tick={{ fontSize: 13 }} />
                  <ReTooltip content={<CustomTooltip />} />
                  <Bar dataKey={activeTab} fill={activeTab === 'damage' ? '#e97171' : activeTab === 'gold' ? '#fbbf24' : activeTab === 'vision_score' ? '#60a5fa' : activeTab === 'damage_taken' ? '#10b981' : '#a78bfa'} radius={[6, 6, 0, 0]} barSize={30}
                    onClick={(d) => handlePlayerClick(d.nickname, true)}>
        ...
        {mainTab === 'champion' && (
          <section style={{ backgroundColor: '#1f2937', padding: '35px', borderRadius: '16px', border: '1px solid #374151' }}>
            <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#ffffff', marginBottom: '24px' }}>⚔️ 챔피언 분석</h2>
            <ChampionAnalysis allStats={allStats} matches={matches} />
          </section>
        )}

        {/* ===== 개인 지표 탭 ===== */}
        {mainTab === 'player' && (
          <section style={{ backgroundColor: '#1f2937', padding: '35px', borderRadius: '16px', border: selectedPlayer ? '2px solid #3b82f6' : '1px solid #374151' }}>

            {/* 개인지표 탭 자체 검색창 */}
            {!selectedPlayer && (
              <div style={{ marginBottom: '30px' }}>
                <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#ffffff', marginBottom: '16px' }}>👤 개인 지표</h2>
                <div style={{ position: 'relative', width: '100%', maxWidth: '500px', margin: '0 auto' }}>
                  <input
                    ...
                    style={{ width: '100%', backgroundColor: '#111827', border: '2px solid #3b82f6', borderRadius: '12px', padding: '15px 20px', color: '#fff', fontSize: '16px', outline: 'none', boxSizing: 'border-box' }}
                    autoFocus
                  />
                </div>
                {playerSearchTerm && (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '12px', marginTop: '16px', maxWidth: '700px', margin: '16px auto 0' }}>
                    {playerSearchResults.map(player => (
                      <div key={player.nickname} onClick={() => { handlePlayerClick(player.nickname, false); setPlayerSearchTerm(''); }} style={{ backgroundColor: '#111827', padding: '14px', borderRadius: '12px', border: '1px solid #374151', cursor: 'pointer', transition: '0.15s' }}
                        onMouseEnter={e => e.currentTarget.style.borderColor = '#3b82f6'}
                        onMouseLeave={e => e.currentTarget.style.borderColor = '#374151'}
                      >
                        <div style={{ fontWeight: 'bold', fontSize: '14px', marginBottom: '4px' }}>{player.nickname}</div>
            ...
            <PlayerReport
              ...
              getRankingsByLine={getRankingsByLine}
            />
          </section>
        )}
...
{mainTab === 'h2h' && (
  <section style={{ backgroundColor: '#1f2937', padding: '35px', borderRadius: '16px', border: '1px solid #374151' }}>
    <h2 style={{ fontSize: '20px', fontWeight: 'bold', color: '#ffffff', marginBottom: '24px' }}>🆚 상대 전적</h2>
    <HeadToHead
      allStats={allStats}
      matches={matches}
      onNavigateToPlayer={(nickname) => {
        handlePlayerClick(nickname, true);
      }}
    />
  </section>
)}
